...
**Decision:** Once a change is locked and exposed to real-world evidence, any repair must remain inside the originally approved intent, scope, acceptance criteria, and validation target. Each repair returns to the same validation it failed, not a new one, with fresh evidence. `docs/TESTING.md` defines the risk-classification policy that governs evidence depth and required audit rigor for each repair; the change's own base tier and specialized profiles are recorded in the active change's canonical `spec.md`, or the approved lightweight approval record, not in `docs/TESTING.md` itself. This governs depth and rigor only — it does not govern how many repair attempts are allowed. The maximum number of bounded repair attempts for a given validation target is instead stated explicitly in the active change's canonical `spec.md`, or in the approved lightweight approval record when a full OpenSpec change is not used, and is tracked using that same record together with the existing `Active change` and `Exact next action` fields of `docs/CURRENT.md`, so it recovers across sessions. Changing an approved attempt bound requires an explicit approved update to that record; it is never inferred, defaulted, or silently raised. If a repair would require changing the locked scope or acceptance criteria, exhausts the approved attempt bound, or leaves risk unresolved, escalate through the existing founder-approval, incident, and manual rollback mechanisms rather than silently expanding scope or looping indefinitely. Independent Codex audit during repair remains governed by the recorded risk policy — mandatory for the High tier, required for Incident work when operationally possible, and otherwise required only when the approved change, the founder, or a documented risk reclassification calls for it; builder-side review never substitutes for it where it is required.

This decision extends the existing scope-discipline rule in `AGENTS.md` ("stay within approved scope") and the risk-reclassification rule in `docs/TESTING.md` ("stop and reclassify... do not finish under the original tier") to the moment after lock, rather than creating a new lifecycle abstraction. It does not introduce a named operating mode, a new report type, or a new recovery schema — the attempt bound is recorded on the same canonical surface already used for risk classification, repair evidence is recorded as a further instance of `verification-report.md` against the same change, and cross-session state is recorded in the existing `Active change` and `Exact next action` fields of `docs/CURRENT.md`.
